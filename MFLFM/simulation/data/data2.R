...
# observation numbers
nobs1 <- nobs2 <- rep(length(te),n) #sample(40:60, n, replace=TRUE)
...
Ymean1 <- Ymean2 <- matrix(NA, length(te), n)
for(i in 1:n){ 
  
  if(i <= n1){
    
    tempu1 <- rnorm(1,    1,  0.3)
    tempu2 <- rnorm(1,    2,  0.6)
    tempu3 <- rnorm(1,    1,  0.3)
    tempu4 <- rnorm(1,    1,  0.3)
    tempu5 <- rnorm(1,    0,  0.6)
    tempu6 <- rnorm(1,    0,  0.9)
    ...
    Ymean1[,i] <- (1 + tempg11/4)*te + (tempu1+tempg22/5) * fval1 + (tempu2+tempg21/5) * fval2 + tempu3 * fval3
    Ymean2[,i] <- (2 + tempg11/3) + (tempu4+tempg32/5) * fval4 + (tempu5+tempg31/5) * fval5 + tempu6 * fval6
    
  }else if(i <= n1+n2){ 
    
    tempu1 <- rnorm(1,    2,  0.6)
    tempu2 <- rnorm(1,    0,  0.3)
    tempu3 <- rnorm(1,    2,  0.6)
    tempu4 <- rnorm(1,    0,  0.3)
    tempu5 <- rnorm(1,    2,  0.6)
    tempu6 <- rnorm(1,    0,  0.9)
    ...
    Ymean1[,i] <- (2 + tempg11/4)*te + (tempu1+tempg22/5) * fval1 + (tempu2+tempg21/5) * fval2 + tempu3 * fval3
    Ymean2[,i] <- (4 + tempg11/3) + (tempu4+tempg32/5) * fval4 + (tempu5+tempg31/5) * fval5 + tempu6 * fval6
    
  }else{
    
    tempu1 <- rnorm(1,    3,  0.9)
    tempu2 <- rnorm(1,    6,  1.8)
    tempu3 <- rnorm(1,    3,  0.9)
    tempu4 <- rnorm(1,    0,  0.5)
    tempu5 <- rnorm(1,    0,  0.6)
    tempu6 <- rnorm(1,    3,  0.9)
    ...
    Ymean1[,i] <- (3 + tempg11/4)*te + (tempu1+tempg22/5) * fval1 + (tempu2+tempg21/5) * fval2 + tempu3 * fval3
    Ymean2[,i] <- (6 + tempg11/3) + (tempu4+tempg32/5) * fval4 + (tempu5+tempg31/5) * fval5 + tempu6 * fval6
  }
}


Ydat1 <- Ydat11 <- Ymean1 + matrix( rnorm(length(te)*n, 0, 0.5), length(te), n )
Ydat2 <- Ydat22 <- Ymean2 + matrix( rnorm(length(te)*n, 0, 0.5), length(te), n )
...
plot(te, Ymean1[,1], type='l', ylim=YLIM, ylab='y', xlab='t', 
     main = paste(figurenum[1], ") Mean Curves of dimension 1 for Simulation case (2)", sep=''), cex.main=1.4, cex.lab=1.3 )
for(i in 1:n1){ lines(te, Ymean1[,i]) }
for(i in (1:n2)+n1 ){ lines(te, Ymean1[,i],col=2) }
for(i in (1:n3)+n1+n2 ){ lines(te, Ymean1[,i], col=4) }


YLIM <- quantile(Ymean2, c(0,1))
plot(te, Ymean2[,1], type='l', ylim=YLIM, ylab='y', xlab='t', 
     main = paste(figurenum[2], ") Mean Curves of dimension 2 for Simulation case (2)", sep=''), cex.main=1.4, cex.lab=1.3 )
for(i in 1:n1){ lines(te, Ymean2[,i]) }
for(i in (1:n2)+n1 ){ lines(te, Ymean2[,i],col=2) }
for(i in (1:n3)+n1+n2 ){ lines(te, Ymean2[,i], col=4) }


YLIM <- quantile(Ydat11, c(0,1))
plot(te, Ydat11[,1], type='l', ylim=YLIM, ylab='y', xlab='t', 
     main = paste(figurenum[3], ") Observed Curves of dimension 1 for Simulation case (2)", sep=''), cex.main=1.4, cex.lab=1.3 )
for(i in 1:n1){ lines(te, Ydat11[,i]) }
for(i in (1:n2)+n1 ){ lines(te, Ydat11[,i],col=2) }
for(i in (1:n3)+n1+n2 ){ lines(te, Ydat11[,i], col=4) }


YLIM <- quantile(Ydat22, c(0,1))
plot(te, Ydat22[,1], type='l', ylim=YLIM, ylab='y', xlab='t', 
     main = paste(figurenum[4], ") Observed Curves of dimension 2 for Simulation case (2)", sep=''), cex.main=1.4, cex.lab=1.3 )
for(i in 1:n1){ lines(te, Ydat22[,i]) }
for(i in (1:n2)+n1 ){ lines(te, Ydat22[,i],col=2) }
