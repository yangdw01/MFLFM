...
set.seed(54)
source("preprocess.R")
# tempresult3 <- getCLUST( burnthin-1, niter, eps,
...
tempresult2[clusters == 1] <- 2
tempresult2[clusters == 2] <- 5
tempresult2[clusters == 14] <- 1
tempresult2[clusters == 12] <- 4
...
points( mcc.indicators[citynum_mcc,geo_list[2]], mcc.indicators[citynum_mcc,geo_list[1]], pch=19, col=tempresult2 )
# text( mcc.indicators[citynum_mcc,geo_list[2]], mcc.indicators[citynum_mcc,geo_list[1]], 1:n, cex=0.8, col=tempresult )
title(main="(i) Canada map")
...
plot( jitter(original_Xdat[,3], factor=10), jitter(original_Xdat[,4], factor=10), pch=19, col=tempresult2, xlab="GDP ($)", ylab="Unemp (%)", main="(g) Socio-economic variables" )
legend(58900,13.6, legend = c("cluster 1", "cluster 2", "cluster 3", "cluster 4", "cluster 5"), col=c(1,2,4,3,5), pch = c(19,19,19,19,19))
...
clusters2 <- tempresult2
monthnum <- c(16, 47, 75, 106, 136, 167, 197,  228, 259, 289, 320, 350)
monthnames <- c("Jan.", "Feb.", "Mar.", "Apr.", "May", "June", "July", "Aug.", "Sep.", "Oct.", "Nov.", "Dec.")

plot(msc1[,1], type='l', ylim=c(8,35), main=expression("(d) MF-LFM1 - O"[3]), ylab=expression("O"[3] * " (ppb)"), xlab='Month', xaxt='n')
axis(1, monthnum, monthnames, labels = F)
text(monthnum, par("usr")[3] - 1, labels = monthnames, srt = 45, pos = 1, xpd = TRUE, cex=0.85)
for(i in sort(unique(tempresult2))){ lines(msc1[,clusters[which(clusters2 == i)[1]]], col=i, lwd=3) }


plot(msc2[,1], type='l', ylim=c(5,27), main=expression("(e) MF-LFM1 - NO"[2]), ylab=expression("NO"[2] * " (ppb)"), xlab='Month', xaxt='n')
axis(1, monthnum, monthnames, labels = F)
text(monthnum, par("usr")[3] - 1, labels = monthnames, srt = 45, pos = 1, xpd = TRUE, cex=0.85)
for(i in sort(unique(tempresult2))){ lines(msc2[,clusters[which(clusters2 == i)[1]]], col=i, lwd=3) }
